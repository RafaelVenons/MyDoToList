...
function NovaAtividade({goHome}) {
  ...
  const { tarefas, setTarefas, metas, setMetas } = useContext(Atividades);
  ...
  const Adiciona = async () => {
    ...
    if (toggle) {
      ...
      try {
        ...
        await AsyncStorage.setItem("@todolist:tarefas", JSON.stringify(data));
        setTarefas(data);
        goHome();
      } catch (error) {
        console.log(error);
      ...
      try {
        ...
        await AsyncStorage.setItem("@todolist:metas", JSON.stringify(data));
        setMetas(data);
        goHome();
      } catch (error) {
        console.log(error);
